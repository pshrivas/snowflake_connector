...
public class SnowflakeV2BaseConnectInfoAdapter extends AbstractINFASDKAdapter {
	...
	/**
	 * Implement this method to set the attribute types for the adapter or to
	 * modify existing connection attribute types.
	 *
	 *************************************
	 * AUTOGENERATED CODE : DO NOT CHANGE
	 *
	 * @param connAttrTypes
	 *            A list of connection attribute types.
	 */
	...
	public void updateConnectInfoAttributeTypes(SDKConnectAttributeType attrib) {
		...
		} else if (attribName.equalsIgnoreCase("password")) {
			attrib.setAttributeCLIDisplayName("password");
		} else if (attribName.equalsIgnoreCase("db")) {
			attrib.setAttributeCLIDisplayName("db");
		} else if (attribName.equalsIgnoreCase("warehouse")) {
			attrib.setAttributeCLIDisplayName("warehouse");
		} else if (attribName.equalsIgnoreCase("account")) {
			attrib.setAttributeCLIDisplayName("account");
		} else if (attribName.equalsIgnoreCase("role")) {
			attrib.setAttributeCLIDisplayName("role");
		} else if (attribName.equalsIgnoreCase("schema")) {
			attrib.setAttributeCLIDisplayName("schema");
		} else if (attribName.equalsIgnoreCase("useCustomURL")) {
			attrib.setAttributeCLIDisplayName("useCustomURL");
		} else if (attribName.equalsIgnoreCase("customURL")) {
			attrib.setAttributeCLIDisplayName("customURL");
		} else if (attribName.equalsIgnoreCase("clientSessionKeepAlive")) {
	...
	/**
	 * This method to provides the presentation details like pages for a
	 * particular consumer type. The pages, groups, and attributes displayed for
	 * the adapter on the consumer page depends on the type of consumer
	 * application that will use the adapter. The default presentation page is
	 * auto generated. This method can be changed only if the adapter needs a
	 * different type of presentation than the default generated code
	 *
	 *************************************
	 * AUTOGENERATED CODE : DO NOT CHANGE
	 *
	 * @param consumerType
	 *            The client application that will use this adapter.
	 *
	 * @return SDKConnectionConsumerInfo consumer pages for the adapter
	 */
	...
	private SDKConnectionConsumerGroup createPage1Group1DFLT() {
		...
		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("user", "USER", "USER_TOOLTIP", true, false, true));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("password", "PASSWORD", "PASSWORD_TOOLTIP", true, false, true));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("db", "DB", "DB_TOOLTIP", true, false, false));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("warehouse", "WAREHOUSE", "WAREHOUSE_TOOLTIP", true, false, false));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("account", "ACCOUNT", "ACCOUNT_TOOLTIP", true, false, true));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("role", "ROLE", "ROLE_TOOLTIP", true, false, false));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("schema", "SCHEMA", "SCHEMA_TOOLTIP", true, false, false));

		group.getConnInfoPresentationControls().add(
				createAttributePresentation("useCustomURL", "USECUSTOMURL", "USECUSTOMURL_TOOLTIP", true, false, true));

		group.getConnInfoPresentationControls()
				.add(createAttributePresentation("customURL", "CUSTOMURL", "CUSTOMURL_TOOLTIP", true, false, false));

		group.getConnInfoPresentationControls().add(createAttributePresentation("clientSessionKeepAlive",
				"CLIENTSESSIONKEEPALIVE", "CLIENTSESSIONKEEPALIVE_TOOLTIP", true, false, true));
	...
	/**
	 * Creates a new SDKConnectionConsumerAttribute object and populates it with
	 * attribute and presentation information.
	 * 
	 * @param attributeName
	 ...
	 * @param toolTip
	 *            Tooltip text to display in the user interface.
	 * @param enabled
	 *            Determines whether the attribute is enabled or disabled on the
	 *            user interface.
	 * @param hasDependentAttr
	 *            Determines whether the attribute has dependent attributes that
	 *            should be updated for changes in this attribute.
	 * @param isMandatory
	 *            Determines whether the attribute is a mandatory attribute.
	 * @return Returns a SDKConnectionConsumerAttribute object which is used on
	 *         the user interface.
	 */
	public SDKConnectionConsumerAttribute createAttributePresentation(String attributeName, String presentationName,
			String toolTip, boolean enabled, boolean hasDependentAttr, boolean isMandatory) {
		...
		attributeTypePresentation.setAttributeType(attributeMap.get(attributeName));
		attributeTypePresentation.getAttributeType().setIsMandatory(isMandatory);
