...
public class SnowflakeV2Connection extends AbstractConnection {
    ...
    public Status openConnection(Map<String, Object> connAttrs){
		...
		if (!useCustomURL) {
		    ...
		    connectionURL = JDBC_URL_PREFIX + "://" + account + ".snowflakecomputing.com" + "/?" 
														+ queryString;
		} else {
			...
			try {
				...
				return new Status(StatusEnum.FAILURE,
						String.format("Failed to parse JDBC URL: %s, Error: %s",
								customURL, e.getMessage()));
			}
     ...
     * 
     * @return the db name (catalog). null if not specified in the connection attributes.
     */
     ...
     * 
     * @return the schema name. null if not specified in the connection attributes.
     */
